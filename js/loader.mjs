    ...
    if (e.key === 'Enter') { // If it's enter
      (message => { // This processes the commmand
        Object.keys(commands).forEach((name) => { // For every command
          if (message.content.startsWith(`-${name}`)) commands[name](message, message.content.substring(2 + name.length).split(' ')) // Run the command if input = command (also provide args)
        })
